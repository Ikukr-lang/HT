...
def get_expected_goals(att_l, att_c, att_r, def_opp_r, def_opp_c, def_opp_l, possession):
    total = 10.0
    ch = total * possession
    return (ch*0.35*prob_score(att_c/def_opp_c) + ch*0.25*prob_score(att_l/def_opp_r) + ch*0.25*prob_score(att_r/def_opp_l) + ch*0.15*0.12)
...
def parse_report_text(text: str):
    ...
    if len(zone_percents) >= 14:
        ...
        for pat in [r"центр.*?(\d+)%", r"center.*?(\d+)%", r"владения в центре.*?(\d+)%"]:
...
<html lang="ru">
<head><meta charset="UTF-8"><title>Hattrick Анализатор</title>
<style>body{font-family:Arial,sans-serif;max-width:800px;margin:40px auto;padding:20px;background:#f8f9fa}h1{color:#28a745;text-align:center}.result{background:#e9f7ef;padding:25px;border-radius:12px;margin:20px 0;border:2px solid #28a745;font-size:18px}form{margin:30px 0;padding:25px;background:white;border-radius:8px;box-shadow:0 2px 15px rgba(0,0,0,0.1)}input[type="file"]{width:100%;padding:12px;margin:10px 0;border:2px solid #ddd;border-radius:6px;font-size:16px}button{padding:14px 40px;background:#28a745;color:white;border:none;border-radius:6px;font-size:18px;cursor:pointer;width:100%}button:hover{background:#218838}.error{color:#dc3545;background:#f8d7da;padding:15px;border-radius:8px;margin:20px 0}</style>
</head>
<body>
